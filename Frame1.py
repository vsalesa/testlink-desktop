...
[wxID_FRAME1, wxID_FRAME1BACKBUTTON, wxID_FRAME1BLOCKBUTTON, 
 wxID_FRAME1CASENAMELIST, wxID_FRAME1FAILBUTTON, wxID_FRAME1FORWARDBUTTON, 
 wxID_FRAME1GETTESTCASES, wxID_FRAME1HTMLWINDOW1, wxID_FRAME1PANEL1, 
...
class Frame1(wx.Frame):
    def _init_ctrls(self, prnt):
        ...
        wx.Frame.__init__(self, id=wxID_FRAME1, name='', parent=prnt,
              pos=wx.Point(362, 45), size=wx.Size(753, 251),
              style=wx.DEFAULT_FRAME_STYLE, title='TestLink Case Executor')
        self.SetClientSize(wx.Size(737, 213))

        self.panel1 = wx.Panel(id=wxID_FRAME1PANEL1, name='panel1', parent=self,
              pos=wx.Point(0, 0), size=wx.Size(737, 213),
...
##        self.CaseNameList.SetSelection(0)
    ...
    def ShowNext(self):
        if self.CurrentCase == len(self.CaseIndex):
            pass
        else:
    ...
    def OnPassButtonButton(self, event):
        if len(self.CaseList) == 0:
            ...
            self.ShowNext()
            update = Backend.UpdateResult(self.CaseIndex[self.CurrentCase], 4809, 'p', 30)
    ...
    def OnFailButtonButton(self, event):
        if len(self.CaseList) == 0:
            ...
            self.ShowNext()
            update = Backend.UpdateResult(self.CaseIndex[self.CurrentCase], 4809, 'f', 30)
            self.CaseNameList.SetItemBackgroundColour(self.CurrentCase, wx.Colour(0, 255, 128))
            self.CaseNameList.SetItemForegroundColour(self.CurrentCase, wx.Colour(0, 255, 128))
            modentry = self.CaseList[self.CurrentCase][0]
            modentry = '[F]' + modentry
    ...
    def OnBlockButtonButton(self, event):
        if len(self.CaseList) == 0:
            ...
            self.ShowNext()
            update = Backend.UpdateResult(self.CaseIndex[self.CurrentCase], 4809, 'B', 30)
            self.CaseNameList.SetItemBackgroundColour(self.CurrentCase, wx.Colour(0, 255, 128))
            self.CaseNameList.SetItemForegroundColour(self.CurrentCase, wx.Colour(0, 255, 128))
            modentry = self.CaseList[self.CurrentCase][0]
            modentry = '[B]' + modentry
